    ...
    for i in range(4):
        for j in range(3):    
            if def_to_check(lst[i][2 - j], lst[i][3 - j]):
                ...
                if lst[i][3-j] in Happy and lst[i][3-j] not in opened_happy:
                    opened_happy.append(lst[i][3-j])
                if lst[i][3-j] in Prime and lst[i][3-j] not in opened_prime:
                    opened_prime.append(lst[i][3-j])
                if lst[i][3-j] in Ulama and lst[i][3-j] not in opened_ulam:
...
def int_to_str(lst, U, P, H):
    ...
    for k in range(len(lst)):
        ...
        for i in range(4):
            ...
            list_of_line.append(color + str(lst[k][i]) + '\033[0m' + ' '*(8 - rm_space))
...
if __name__ == '__main__':
    ...
    while game_is_playing:
        ...
        print('\33[31mList of  Ulam numbers, you have opened:' + str(sorted(opened_ulam)))
        print('\33[32mList of Prime numbers, you have opened:' + str(sorted(opened_prime)))
        print('\33[34mList of Happy numbers, you have opened:' + str(sorted(opened_happy)))
